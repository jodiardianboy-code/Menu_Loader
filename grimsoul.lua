--========================================================
-- 🧭 Draboy script v1 - MODULAR VERSION
--========================================================

-- ==================== GLOBAL VARIABLES ====================
local GLOBAL_BACKUP_DATA = nil
local ItemQueue = {}
local RESTORE_DELAY_MS = 8000
...
local ITEM_LIST_DATA = {
 ...
 ["Bags Collection"] = {
        ["Armor Bag"] = "armor_bag",
        ["Backpack 10"] = "backpack_10",
        ["Backpack 15"] = "backpack_15",
        ["Backpack 5"] = "backpack_5",
        ["Backpack Black"] = "backpack_black",
        ["Backpack Red"] = "backpack_red",
        ["Backpack Violet"] = "backpack_violet",
    ...
    ["Items"] = {
        ...
        ["Boat item rare [not ready in game]"] = {
            ["Mast"] = "mast",
            ["Sail"] = "sail",
            ["Tiller"] = "tiller",
            ["Yard"] = "yard",
...
function Search.show_results(results, query)
    ...
    for i, result in ipairs(results) do
        ...
        if name_lower:find("sword") or name_lower:find("blade") then
            icon = "🗡️"
        elseif name_lower:find("shield") then
            icon = "🛡️"
        elseif name_lower:find("bow") then
            icon = "🏹"
        elseif name_lower:find("potion") then
            icon = "🧪"
        elseif name_lower:find("ore") or name_lower:find("ingot") then
            icon = "⛏️"
        elseif name_lower:find("wood") then
            icon = "🪵"
        elseif name_lower:find("food") then
            icon = "🍗"
        end
...
local function count_active_mods()
    ...
    if STATE.DUP_V2 then count = count + 1 end    -- V2
...
-- ==================== DUPLICATE V1 (5000) - ASLI ====================
function toggle_duplicate_v1()
    if STATE.DUP then
        sv(CACHE.DUP)
        CACHE.DUP = {}
        STATE.DUP = false
        gg.toast("🔴 Duplicate (5000) OFF")
        return
    end
    ...
    if not METHOD.DUP then
        ...
        if #METHOD.DUP == 0 then
            gg.alert("❌ get__amount method not found")
            METHOD.DUP = nil
            return
        end
    end
    ...
    sv(patch)
    STATE.DUP = true
    gg.toast("🟢 Duplicate (5000) ON")
end

-- ==================== DUPLICATE V2 (20) - VERSI ANDA ====================
function toggle_duplicate()
    if STATE.DUP_V2 then
        sv(CACHE.DUP_V2)
        CACHE.DUP_V2 = {}
        STATE.DUP_V2 = false
        gg.toast("🔴 Duplicate (20) OFF")
        return
    end

    if not METHOD.DUP_SET then
        METHOD.DUP_SET = findMethod("LimitedInventoryStack", "SetAmount")
        if not METHOD.DUP_SET or #METHOD.DUP_SET == 0 then
            gg.alert("❌ SetAmount not found")
            return
        end
    end
    ...
    for _, a in ipairs(METHOD.DUP_SET) do
        -- backup instruksi compare (address+8)
        table.insert(CACHE.DUP_V2, {
            address = a + 8, 
            flags = gg.TYPE_DWORD, 
            value = gv(a + 8, gg.TYPE_DWORD)
        })
        
        -- MOV W1, W0 (force newAmount = currentAmount)
        table.insert(patch, {
            address = a + 8, 
            flags = gg.TYPE_DWORD, 
            value = "hE10300AA"
    ...
    sv(patch)
    STATE.DUP_V2 = true
    gg.toast("🟢 Duplicate (20) ON - Fixed (usable & numbered)")
end
...
function toggle_research_craft()
    ...
    if not METHOD.RESEARCH then
        ...
        if #METHOD.RESEARCH == 0 then
            gg.alert("❌ get_canCraft not found")
            METHOD.RESEARCH = nil
            return
        end
    end
...
function toggle_instant_kill()
    ...
    -- Cari method GetWeaponDamageBonus
    if not METHOD.INSTANT_KILL then
        gg.toast("🔍 Scanning for damage method...")
        METHOD.INSTANT_KILL = findMethod("WeaponAttackActivity", "GetWeaponDamageBonus")
        
        -- Jika tidak ketemu, cari method damage lainnya
        if #METHOD.INSTANT_KILL == 0 then
            METHOD.INSTANT_KILL = findMethod("WeaponAttackActivity", "CalculateDamage")
        end
        if #METHOD.INSTANT_KILL == 0 then
            METHOD.INSTANT_KILL = findMethod("WeaponAttackActivity", "CalculateRawDamage")
        end
        if #METHOD.INSTANT_KILL == 0 then
            METHOD.INSTANT_KILL = findMethod("WeaponAttackActivity", "GetBaseExtraDamage")
        end
        
        if #METHOD.INSTANT_KILL == 0 then
            gg.alert("❌ Damage methods not found!\n\nTry:\n1. Enter combat first\n2. Attack an enemy once\n3. Try again")
            METHOD.INSTANT_KILL = nil
            return
        end
    end

    -- Backup dan patch untuk INSTANT KILL (9999 damage)
    CACHE.INSTANT_KILL = {}
    local patch = {}
    
    local methods_found = #METHOD.INSTANT_KILL
    gg.toast("✅ Found " .. methods_found .. " damage method(s)")
    
    for _, a in ipairs(METHOD.INSTANT_KILL) do
        -- Backup original instructions (16 bytes = 4 instructions)
        for i = 0, 3 do
            local addr = a + (i * 4)
        ...
        -- PATCH: mov w0, #9999 (0x270F) ; ret
        -- ARM64: mov w0, #9999 = 0F 27 80 52 (little endian: 0x5227800F)
        -- ret = C0 03 5F D6 (little endian: 0xD65F03C0)
        
        table.insert(patch, {
            address = a,
            flags = gg.TYPE_DWORD,
            value = "h0F278052"  -- mov w0, #9999
        })
...
function toggle_free_assemble()
    ...
    if not METHOD.FREE_ASSEMBLE then
        ...
        if not METHOD.FREE_ASSEMBLE or #METHOD.FREE_ASSEMBLE == 0 then
            gg.alert("❌ Cannot find BuildingCollection methods!\n\n" ..
...
local function mod_sub_menu()
    while true do
        ...
        local menu = gg.choice({
            ...
            "✂️ Duplicate V1 " .. (STATE.DUP and "✅" or "🔴"),           -- Index 3
            "✂️ Duplicate V2 " .. (STATE.DUP_V2 and "✅" or "🔴"),         -- Index 4
            "✂️ Split Weapon " .. (STATE.SPLIT and "✅" or "🔴"),               -- Index 5
            "🔒 Lock Durability " .. (STATE.DURA and "✅" or "🔴"),             -- Index 6
            "⚡️ Zero Energy Travel " .. (STATE.FREE and "✅" or "🔴"),         -- Index 7
            "🛠️ Free Craft " .. (STATE.RESEARCH and "✅" or "🔴"),             -- Index 8
            "🚀 Fast Crafting " .. (STATE.CRAFT_SPEED and "✅" or "🔴"),        -- Index 9
            "🔨 Free Assemble " .. (STATE.FREE_ASSEMBLE and "✅" or "🔴"),      -- Index 10
            "════════════════════════",       -- Index 11
            "🔙 Back to Main Menu"            -- Index 12
        ...
        elseif menu == 4 then toggle_duplicate()
        elseif menu == 5 then toggle_split()
        elseif menu == 6 then toggle_dura()
        elseif menu == 7 then toggle_free()
        elseif menu == 8 then toggle_research_craft()
        elseif menu == 9 then toggle_crafting_speed()
        elseif menu == 10 then toggle_free_assemble()
        elseif menu == 12 then 
            return
        end
    end
...
-- ============================================================
-- ✨ DRABOY GRIM SOUL SURVIVAL TOOLS V1
-- 📅 Release: 19 December 2025 | Game: 7.9.0 (64-bit)
-- ============================================================
...
local function show_welcome()
 ...
 ✨ DRABOY GRIM SOUL TOOLS V2 
 📅 Version: 7.9.0 (64-bit)
 🛠️ Release: 19 Dec 2025
 ───────────────────────────────

 📝 UPDATE LOG:
 ───────────────────────────────
 🔸 Remove Pets System cause unstable
───────────────────────────────
...
───────────────────────────────
 ...
 ✅ Say thanks with
 ✅ Subscribe: Yt @DraboyGaming
 ✅ if want this script exist
───────────────────────────────
...
local function exit_script()
    ...
    if active_mods > 0 then
        ...
        if confirm == 1 then
            ...
            if STATE.DUP_V2 then toggle_duplicate() end
...
----------------------------------------------------
-- ACTIVATE ALL MODS (SIMPLE VERSION)
----------------------------------------------------
local function activate_all_mods_simple()
    -- Konfirmasi sekali saja
    local confirm = gg.alert(
        "⚡ ACTIVATE ALL MODS\n" ..
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n" ..
        "Activate all 9 mods at once?\n\n" ..
        "• Instant Kill (9999 Damage)\n" ..
        --"• Duplicate V1 (5000)\n" ..
        "• Duplicate V2 (20)\n" ..
        "• Split Weapon\n" ..
        "• Infinite Durability\n" ..
        ...
        "• Free Craft Research\n" ..
        "• Fast Crafting (1s)\n" ..
        "• Free Building Assemble",
        "✅ ACTIVATE ALL",
        "❌ CANCEL"
    )
    
    if confirm ~= 1 then
        gg.toast("❌ Activation cancelled")
        return
    end
    
    gg.toast("🚀 Activating all mods...")
    
    -- Hitung mod yang belum aktif
    local remaining = 8
    local activated = 0
    
    -- Aktifkan semua mod dengan progress toast
    if not STATE.INSTANT_KILL then
        toggle_instant_kill()
        activated = activated + 1
        remaining = remaining - 1
        gg.toast("✅ Instant Kill ON | " .. remaining .. " left")
        gg.sleep(200)
    end
    
   -- if not STATE.DUP then
     --   toggle_duplicate_v1()
      --  activated = activated + 1
      --  remaining = remaining - 1
     --   gg.toast("✅ Duplicate V1 ON | " .. remaining .. " left")
      --  gg.sleep(200)
  --  end
    
    if not STATE.DUP_V2 then
        toggle_duplicate()
        activated = activated + 1
        remaining = remaining - 1
        gg.toast("✅ Duplicate V2 ON | " .. remaining .. " left")
        gg.sleep(200)
    end
    ...
    if not STATE.SPLIT then
        ...
        activated = activated + 1
        remaining = remaining - 1
        gg.toast("✅ Split Weapon ON | " .. remaining .. " left")
        gg.sleep(200)
    end
    
    if not STATE.DURA then
        toggle_dura()
        activated = activated + 1
        remaining = remaining - 1
        gg.toast("✅ Durability ON | " .. remaining .. " left")
        gg.sleep(200)
    end
    
    if not STATE.FREE then
        toggle_free()
        activated = activated + 1
        remaining = remaining - 1
        gg.toast("✅ Free Energy ON | " .. remaining .. " left")
        gg.sleep(200)
    end
    
    if not STATE.RESEARCH then
        toggle_research_craft()
        activated = activated + 1
        remaining = remaining - 1
        gg.toast("✅ Free Craft ON | " .. remaining .. " left")
        gg.sleep(200)
    end
    
    if not STATE.CRAFT_SPEED then
        toggle_crafting_speed()
        activated = activated + 1
        remaining = remaining - 1
        gg.toast("✅ Fast Crafting ON | " .. remaining .. " left")
        gg.sleep(200)
    end
    
    if not STATE.FREE_ASSEMBLE then
        toggle_free_assemble()
        activated = activated + 1
        remaining = remaining - 1
        gg.toast("✅ Free Assemble ON | " .. remaining .. " left")
        gg.sleep(200)
    end
    
    -- Tampilkan summary cepat
    if activated > 0 then
        gg.toast("✨ " .. activated .. " mods activated!")
        gg.sleep(800)
        gg.toast("🎮 Ready to play!")
    else
        gg.toast("⚠️ All mods already active!")
    end
end

----------------------------------------------------
-- DEACTIVATE ALL MODS (SIMPLE VERSION)
----------------------------------------------------
local function deactivate_all_mods_simple()
    -- Konfirmasi sekali saja
    local confirm = gg.alert(
        "🛑 DEACTIVATE ALL MODS\n" ..
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n" ..
        "Turn OFF all active mods?\n\n" ..
        "Restores original game behavior",
        "✅ DEACTIVATE ALL",
        "❌ CANCEL"
    )
    
    if confirm ~= 1 then
        gg.toast("❌ Deactivation cancelled")
        return
    end
    ...
    if active_count == 0 then
        gg.toast("⚠️ No active mods to deactivate")
        return
    end
    
    gg.toast("🔄 Deactivating " .. active_count .. " mods...")
    
    local deactivated = 0
    local remaining = active_count
    
    -- Deactivate semua mod yang aktif
    if STATE.INSTANT_KILL then
        toggle_instant_kill()
        deactivated = deactivated + 1
        remaining = remaining - 1
        gg.toast("🔴 Instant Kill OFF | " .. remaining .. " left")
        gg.sleep(200)
    end
    ...
    if STATE.DUP then
        ...
        deactivated = deactivated + 1
        remaining = remaining - 1
        gg.toast("🔴 Duplicate V1 OFF | " .. remaining .. " left")
        gg.sleep(200)
    end
    ...
    if STATE.DUP_V2 then
        ...
        deactivated = deactivated + 1
        remaining = remaining - 1
        gg.toast("🔴 Duplicate V2 OFF | " .. remaining .. " left")
        gg.sleep(200)
    end
    ...
    if STATE.SPLIT then
        ...
        deactivated = deactivated + 1
        remaining = remaining - 1
        gg.toast("🔴 Split Weapon OFF | " .. remaining .. " left")
        gg.sleep(200)
    end
    ...
    if STATE.DURA then
        ...
        deactivated = deactivated + 1
        remaining = remaining - 1
        gg.toast("🔴 Durability OFF | " .. remaining .. " left")
        gg.sleep(200)
    end
    ...
    if STATE.FREE then
        ...
        deactivated = deactivated + 1
        remaining = remaining - 1
        gg.toast("🔴 Free Energy OFF | " .. remaining .. " left")
        gg.sleep(200)
    end
    ...
    if STATE.RESEARCH then
        ...
        deactivated = deactivated + 1
        remaining = remaining - 1
        gg.toast("🔴 Free Craft OFF | " .. remaining .. " left")
        gg.sleep(200)
    end
    ...
    if STATE.CRAFT_SPEED then
        ...
        deactivated = deactivated + 1
        remaining = remaining - 1
        gg.toast("🔴 Fast Crafting OFF | " .. remaining .. " left")
        gg.sleep(200)
    end
    ...
    if STATE.FREE_ASSEMBLE then
        ...
        deactivated = deactivated + 1
        remaining = remaining - 1
        gg.toast("🔴 Free Assemble OFF | " .. remaining .. " left")
        gg.sleep(200)
    end
    
    gg.toast("✅ " .. deactivated .. " mods deactivated!")
    gg.sleep(500)
    gg.toast("🔒 Game restored to normal")
end
    ...
    while true do
        ...
        if gg.isVisible(true) then
            ...
            local menu = gg.choice({
                ...
                "⚡ MOD CONTROL" .. mod_status, -- Header baru (Index 5)
                " ╰▶ 🔥 ACTIVATE ALL MODS", -- (Index 6)
                " ╰▶ 🛑 DEACTIVATE ALL MODS", -- (Index 7)
                " ╰▶ 🛠️ TOGGLE INDIVIDUAL MODS", -- (Index 8)
                "════════════════════════════", -- index 9
                
                "⚙️ Settings",     -- Header (Index 10
                " ╰▶ 🛠️ System Settings", --  (Index 11
            ...
            elseif menu == 6 then activate_all_mods_simple()  -- ACTIVATE ALL (SIMPLE)
            elseif menu == 7 then deactivate_all_mods_simple() -- DEACTIVATE ALL (SIMPLE)
            elseif menu == 8 then mod_sub_menu()             -- MOD MENU Individual
            elseif menu == 11 then Settings.show()            -- 🛠️ System Settings
            elseif menu == 12 then exit_script()              -- 🔴 Exit Script
            end
        end
    end
end
