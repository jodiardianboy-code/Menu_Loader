...
local function ShowWelcomeMessage()
    local response = gg.alert(
        ...
        "⚠️ Important:\n" ..
        "━━━━━━━━━━━━━━━━━━━\n" ..
        "⚠️ Dont Be too Greedy, use as needed \n" ..
        "⚠️ Sometime u just need enjoy the game \n" ..
        "⚠️ Use at Your Risk !\n" ..
        "⚠️ Just support my channel if you like this script\n" ..
        "⚠️ Ytb @DraboyGaming \n\n" ..
...
local function explorer()
    ...
    local mode_sel = gg.choice({
        "🧭 Explorer Replace 1 time",
        "🔁 Explorer Multi Replace (Chain Mode)",
        "🔍 Find Item In Explorer (Search Bar)",
        "✍️ Input ID UTF16 Manual",
        "⚙️ Setting Delay Time (Chain Mode)",
        "⬅️ Back to Main Menu"
...
while true do
    ...
    if gg.isVisible(true) then
        ...
        local c = gg.choice({
            "💫 Menu Item Replacemen",
            " ╰▶🤖 STEP ONE: " .. (_G.CURRENT_ACTIVE_NAME and "✅ " .. _G.CURRENT_ACTIVE_NAME or "🔍 Select Item"),
            " ╰▶🧭 ITEM EXPLORER",
              restore_status,
            "",
            "✨ Other Menu",
        ...
        if c == 2 then
            ...
            local step_one_menu = gg.choice({
                ...
                "",
                "⭐ ADVANCED",
                " ╰▶📁 Choose from Item Explorer",
                " ╰▶🔍 Find Item (Search Bar)",
                " ╰▶✍️ Input ID Manual",
                "",
                " ╰▶⬅️ Back to Main Menu"
            }, nil, "🤖 STEP ONE - Select Scan Method")
            
            if step_one_menu == 2 then
                AutoScanAndPointerSuite("empty_bottle", "Empty Bottle")
            elseif step_one_menu == 3 then
                AutoScanAndPointerSuite("resource_fiber", "Fiber")
            elseif step_one_menu == 4 then
                AutoScanAndPointerSuite("resource_rope", "Rope")
        ...
        -- Jika user tidak memilih atau memilih garis pemisah, loop akan berlanjut
    end
end
